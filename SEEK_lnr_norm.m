...
function [A,S]=Mstep(oldA,oldS,p_lt)
global Nwork Ntask
err = NaN;
maxIter=40;
TOL=1e-6;
...
for iter =1:maxIter
    ...
    elseif iter>=maxIter
        ...
        [g_A,g_S]=update_diff(A,S,p_lt);%ÌÝ¶È
        err=(norm(g_A)^2+norm(g_S)^2)/(Nwork+Ntask);
        alpha=1;
        for iter2=1:maxIter
            if iter2>=maxIter
                A
                ...
                g_A
                g_S
                error('iter2 in argQ reached to maxIter');
            end
            A_tem=A+g_A*alpha;
            S_tem=S+g_S*alpha;
            Q_tem = Qfun(A_tem,S_tem,p_lt);
            if Q_tem > Q
                A = A_tem;
                S = S_tem;
                Q =Q_tem;
